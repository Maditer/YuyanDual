...
class SherpaNcnn(
    ...
    fun acceptWaveform(samples: FloatArray) = acceptWaveform(ptr, samples = samples, sampleRate = config.featConfig.sampleRate)
    fun isReady() = isReady(ptr)
    fun decode() = decode(ptr)
    fun isEndpoint() = isEndpoint(ptr)
    fun reset(recreate: Boolean = false) = reset(ptr, recreate)
    val text: String get() = getText(ptr)
    
    fun destroy() = delete(ptr)
