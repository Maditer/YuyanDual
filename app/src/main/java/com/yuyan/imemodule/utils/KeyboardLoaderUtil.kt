...
class KeyboardLoaderUtil private constructor() {
    ...
    private fun getSoftKeyboard(rows: List<List<SoftKey>>, isNumberRow: Boolean): SoftKeyboard {
        ...
        // 侧边栏占位键检测（用于保留左右侧边栏空间，避免下方行与侧边栏重叠）
        val sidebarKey = rows.flatten().firstOrNull { it.code == InputModeSwitcherManager.USER_DEF_KEYCODE_LEFT_SYMBOL_12 }
        val hasSidebar = sidebarKey != null
        val sidebarLeftF = sidebarKey?.mLeftF ?: 0f
        val sidebarWidthF = sidebarKey?.widthF ?: 0f
        val sidebarRightF = sidebarLeftF + sidebarWidthF
        // 通过占位键的left位置判断侧边栏在左侧还是右侧
        val sidebarOnLeft = hasSidebar && sidebarLeftF < 0.5f
        for (rowBean in rows) {
            lastKeyTop = lastKeyBottom  // 新行top为上一行bottom
            lastKeyRight = 0f // 去除行起始的固定左侧空隙
            ...
            // 行宽归一化：将该行所有按键的left/width按实际行宽拉伸到目标范围
            val minLeft = rowBean.minOf { it.mLeftF }
            val maxRight = rowBean.maxOf { it.mLeftF + it.widthF }
            val span = (maxRight - minLeft).takeIf { it > 0f } ?: 1f
            val env = EnvironmentSingleton.instance
            val rowHasSidebarKey = rowBean.any { it.code == InputModeSwitcherManager.USER_DEF_KEYCODE_LEFT_SYMBOL_12 }
            if (hasSidebar) {
                val reservedStart = if (sidebarOnLeft) sidebarRightF else 0f
                val reservedEnd = if (sidebarOnLeft) 1f else sidebarLeftF
                val targetSpan = (reservedEnd - reservedStart).takeIf { it > 0f } ?: 1f
                if (rowHasSidebarKey) {
                    // 含占位键：保持行内既有位置与宽度，仅更新像素尺寸
                    for (key in rowBean) {
                        key.setSkbCoreSize(env.skbWidth, env.skbHeight)
                    }
                } else {
                    // 手写键盘的非占位行：不受侧边栏约束，底部功能行铺满，单键（删除）右对齐
                    if (mSkbValue == InputModeSwitcherManager.MASK_SKB_LAYOUT_HANDWRITING) {
                        if (rowBean.size == 1) {
                            val key = rowBean[0]
                            val newLeftF = 1f - key.widthF
                            key.setKeyDimensions(newLeftF, key.mTopF, key.heightF)
                            key.setSkbCoreSize(env.skbWidth, env.skbHeight)
                        } else {
                            for (key in rowBean) {
                                val newLeftF = (key.mLeftF - minLeft) / span
                                val newWidthF = key.widthF / span
                                key.setKeyDimensions(newLeftF, key.mTopF, key.heightF)
                                key.widthF = newWidthF
                                key.setSkbCoreSize(env.skbWidth, env.skbHeight)
                            }
                        }
                    } else {
                        // 行不含占位键：若已在可用区则不处理，否则优先平移；必要时再整体缩放以适配
                        val alreadyReserved = (minLeft >= reservedStart - 1e-6f && maxRight <= reservedEnd + 1e-6f)
                        if (alreadyReserved) {
                            for (key in rowBean) {
                                key.setSkbCoreSize(env.skbWidth, env.skbHeight)
                            }
                        } else {
                            val delta = reservedStart - minLeft
                            var needScale = false
                            // 预判平移后是否会越界
                            for (key in rowBean) {
                                val testLeft = key.mLeftF + delta
                                val testRight = testLeft + key.widthF
                                if (testLeft < reservedStart - 1e-6f || testRight > reservedEnd + 1e-6f) {
                                    needScale = true
                                    break
                                }
                            }
                            if (!needScale) {
                                ...
                                for (key in rowBean) {
                                    val newLeftF = reservedStart + (key.mLeftF - minLeft) * scale
                                    val newWidthF = key.widthF * scale
                                    key.setKeyDimensions(newLeftF, key.mTopF, key.heightF)
                                    key.widthF = newWidthF
                                    key.setSkbCoreSize(env.skbWidth, env.skbHeight)
                                }
                ...
                // 无侧边栏：文本编辑两键行左右对齐且宽度匹配“全选”；其他默认归一化
                if (mSkbValue == InputModeSwitcherManager.MASK_SKB_LAYOUT_TEXTEDIT && rowBean.size == 2) {
                    val leftKey = rowBean[0]
                    val rightKey = rowBean[1]
                    leftKey.setKeyDimensions(0.25f, leftKey.mTopF, leftKey.heightF)
                    leftKey.widthF = 0.2475f
                    leftKey.setSkbCoreSize(env.skbWidth, env.skbHeight)
                    rightKey.setKeyDimensions(0.75f, rightKey.mTopF, rightKey.heightF)
                    rightKey.widthF = 0.2475f
                    rightKey.setSkbCoreSize(env.skbWidth, env.skbHeight)
                } else {
                    for (key in rowBean) {
                        val newLeftF = (key.mLeftF - minLeft) / span
                        val newWidthF = key.widthF / span
