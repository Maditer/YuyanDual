...
open class SettingsActivity : AppCompatActivity() {
    ...
    override fun onStart() {
        super.onStart()
        // 若已同意隐私政策且当前未选择语燕为默认输入法，则跳转到引导页的“选择输入法”页面
        if (AppPrefs.getInstance().internal.privacyPolicySure.getValue() && !InputMethodUtil.isSelected()) {
            startActivity<SetupActivity>()
            // 不再主动结束当前设置页，确保选择完成后返回到设置页面
            // finish()
        }
    }
